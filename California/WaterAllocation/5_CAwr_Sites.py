...
#Purpose: To extract CA site information and population dataframe for WaDE_QA 2.0.
#Notes: asdf
...
outdf['CoordinateMethodCV'] = df['LOCATION_METHOD']
...
print("SiteName")
outdf['SiteName'] = df.apply(lambda row: assignSiteName(row['DIVERSION_SITE_NAME']), axis=1)

print("SiteNativeID")
outdf['SiteNativeID'] = df.apply(lambda row: assignSiteNativeID(row['POD_ID']), axis=1)
