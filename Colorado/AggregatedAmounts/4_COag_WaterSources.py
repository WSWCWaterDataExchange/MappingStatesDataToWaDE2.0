...
os.chdir(workingDir)
fileInput = "RawinputData/P_COagg.csv"
...
outdf['WaterSourceName'] = df['Component Name'].str.strip()

print("WaterSourceNativeID")  # has to be one of the last, need length of created outdf
outdf['WaterSourceNativeID'] = df['Component ID'].str.strip()
...
outdf = outdf.drop_duplicates(subset=['WaterSourceName', 'WaterSourceNativeID', 'WaterSourceTypeCV']).reset_index(drop=True) # Going to only use WaterSourceNativeID, temp fix.
##############################
