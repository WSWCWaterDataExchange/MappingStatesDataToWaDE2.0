...
#Purpose: To extract AZ water right information and population dataframe for WaDE_QA 2.0.
...
os.chdir(workingDir)
DM_fileInput = "RawinputData/P_ArizonaMaster.csv"
...
# For fixing AllocationOwner
def fixAllocationOwner(colrowValue):
    if colrowValue == '' or pd.isnull(colrowValue):
        outList = ''
    else:
        strVal = str(colrowValue)
...
outdf['WaterSourceUUID'] = df_DM.apply(lambda row: retrieveWaterSourceUUID(row['in_WaterSourceName']), axis=1)
print(WaterSourceUUIDdict)
...
outdf.DataPublicationDate = "12/01/2020"
...
mask = outdf100.loc[ (outdf100['ExemptOfVolumeFlowPriority'] == 0) &
                     ((outdf100["AllocationPriorityDate"].isnull()) | (outdf100["AllocationPriorityDate"] == '') | (outdf100["AllocationPriorityDate"].str.contains(',')) ) ].assign(ReasonRemoved='Bad AllocationPriorityDate').reset_index()
if len(mask.index) > 0:
    dfpurge = dfpurge.append(mask)
    dropIndex = outdf100.loc[ (outdf100['ExemptOfVolumeFlowPriority'] == 0) &
                              ((outdf100["AllocationPriorityDate"].isnull()) | (outdf100["AllocationPriorityDate"] == '') | (outdf100["AllocationPriorityDate"].str.contains(','))) ]
    outdf100 = outdf100.drop(dropIndex)
    outdf100 = outdf100.reset_index(drop=True)
