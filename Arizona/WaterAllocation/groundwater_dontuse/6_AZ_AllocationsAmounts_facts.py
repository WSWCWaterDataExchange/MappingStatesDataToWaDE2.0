...
#Author: Ryan James
#Purpose: To extract AZ allocation use information and population dataframe WaDEQA 2.0.
...
os.chdir(workingDir)
M_fileInput = "RawinputData/P_ArizonaMaster.csv"
...
AllowTypeDict = {
"EXEMPT":"Exempt",
"NON-EXEMPT - SERVICE":"NON Exempt Service",
"NON-EXEMPT":"NON Exempt",
"ENV - MONITOR":"ENV Monitor",
"NON-EXEMPT - NON-SERVICE":"NON Exempt Non Service",
"NON-EXEMPT - WITHDRAWAL PERMIT":"NON Exempt Withdrawal Permit",
"NON- EXEMPT - AQUIFER STORAGE & RECOVERY (ASR)":"NON Exempt Aquifer Storage Recovery",
"ENV - PIEZOMETER":"ENV Piezometer",
"NON-EXEMPT - RECOVERY":"NON Exempt Recovery",
"ENV - INJECTION":"ENV Injection",
"SPCL - CATHODIC PROTECTION":"SPCL Cathodic Protection",
"NON-EXEMPT - REPLACEMENT WELL IN NEW LOCATION":"NON Exempt Replacement Well in New Location",
"SPCL - EXPLORATION":"SPCL Exploration",
"SPCL - MINERAL EXPLORATION":"SPCL Mineral Exploration",
"ENV - MONITOR OR PIEZOMETER":"ENV Monitor or Piezometer",
"EXEMPT - DOMESTIC STOCK":"EXEMPT Domestic Stock",
"EXEMPT - NON-DOMESTIC":"EXEMPT NON Domestic",
"SPCL - GEOTECHNICAL":"SPCL Geotechnical",
"SPCL - VADOSE ZONE FOR RECHARGE":"SPCL Vadose Zone for Recharge",
"ENV - VADOSE ZONE":"ENV Vadose Zone",
"SPCL - OTHER":"SPCL Other"}
def assignAllocationTypeCV(colrowValue):
    if colrowValue == '' or pd.isnull(colrowValue):
        outList = 'AZ Unspecified'
    else:
...
outdf.MethodUUID = "ADWR_Water Rights"
...
outdf['SiteUUID'] = df_DM.apply(lambda row: retrieveSiteUUID(row['SITE_ID']), axis=1)

print("VariableSpecificUUID")  # Hardcoded
outdf.VariableSpecificUUID = "ADWR_Allocation All"

print("WaterSourceUUID")  # Everything is considered fresh, groundwater
outdf.WaterSourceUUID = "AZ_1"

print("AllocationAmount")
outdf['AllocationAmount'] = df_DM['PUMPRATE']
...
outdf['AllocationMaximum'] = df_DM['TESTEDRATE']

print("AllocationNativeID")  # Will use this with a .groupby() statement towards the ends.
outdf['AllocationNativeID'] = df_DM['REGISTRY_ID'].astype(str) # Native dbtype is float. Need to return this value as a string

print("AllocationOwner")  # Pre-process code
outdf['AllocationOwner'] = df_DM['OWNER_NAME']

print("AllocationPriorityDate")
outdf['AllocationPriorityDate'] = df_DM['APPLICATION_DATE']
...
outdf['AllocationTypeCV'] = df_DM.apply(lambda row: assignAllocationTypeCV(row['WELLTYPE']), axis=1)

print("BeneficialUseCategory")
outdf['BeneficialUseCategory'] = df_DM.apply(lambda row: assignBenUseCategory(row['WATER_USE']), axis=1)
...
outdf.DataPublicationDate = "04/06/2020"
