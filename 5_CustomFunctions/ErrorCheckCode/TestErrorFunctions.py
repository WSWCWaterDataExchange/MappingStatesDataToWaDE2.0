...
#     mask = dfx.loc[dfx["Geometry"].apply(validGeoFunc)].assign(ReasonRemoved='Incomplete Geometry').reset_index()
#     if len(mask.index) > 0:
#         outmaskColumn = ["ReasonRemoved", "WaDEUUID", "RowIndex", "IncompleteField_1", "IncompleteField_2"]
#         outmaskdf = pd.DataFrame(columns=outmaskColumn)
#         outmaskdf['ReasonRemoved'] = mask['ReasonRemoved']
#         outmaskdf['WaDEUUID'] = mask['WaDEUUID']
#         outmaskdf['RowIndex'] = mask['index']
#         outmaskdf['IncompleteField_1'] = mask['Geometry']
#         dfy = dfy.append(outmaskdf)
#
...
def RegulatoryOverlayUUID_RE_Check(dfx, dfy):
    mask = dfx.loc[(dfx["RegulatoryOverlayUUID"].isnull()) |
        ...
        ReasonRemoved='Incomplete RegulatoryOverlayUUID').reset_index()
    if len(mask.index) > 0:
        dfy = dfy.append(mask)
...
def OversightAgency_RE_Check(dfx, dfy):
    mask = dfx.loc[(dfx["OversightAgency"].isnull()) |
        ...
        ReasonRemoved='Incomplete OversightAgency').reset_index()
    if len(mask.index) > 0:
        dfy = dfy.append(mask)
...
def RegulatoryDescription_RE_Check(dfx, dfy):
    mask = dfx.loc[(dfx["RegulatoryDescription"].isnull()) |
        ...
        ReasonRemoved='Incomplete RegulatoryDescription').reset_index()
    if len(mask.index) > 0:
        dfy = dfy.append(mask)
...
def RegulatoryName_RE_Check(dfx, dfy):
    mask = dfx.loc[(dfx["RegulatoryName"].isnull()) |
        ...
        ReasonRemoved='Incomplete RegulatoryName').reset_index()
    if len(mask.index) > 0:
        dfy = dfy.append(mask)
...
def RegulatoryOverlayNativeID_RE_Check(dfx, dfy):
    mask = dfx.loc[dfx["RegulatoryOverlayNativeID"].astype(str).str.len() > 250].assign(
        ReasonRemoved='Incomplete RegulatoryOverlayNativeID').reset_index()
    if len(mask.index) > 0:
        dfy = dfy.append(mask)
...
def RegulatoryStatusCV_RE_Check(dfx, dfy):
    mask = dfx.loc[(dfx["RegulatoryStatusCV"].isnull()) |
        ...
        ReasonRemoved='Incomplete RegulatoryStatusCV').reset_index()
    if len(mask.index) > 0:
        dfy = dfy.append(mask)
...
def RegulatoryStatute_RE_Check(dfx, dfy):
    mask = dfx.loc[dfx["RegulatoryStatute"].str.len() > 500].assign(
        ReasonRemoved='Incomplete RegulatoryStatute').reset_index()
    if len(mask.index) > 0:
        dfy = dfy.append(mask)
...
def RegulatoryStatuteLink_RE_Check(dfx, dfy):
    mask = dfx.loc[dfx["RegulatoryStatuteLink"].str.len() > 500].assign(
        ReasonRemoved='Incomplete RegulatoryStatuteLink').reset_index()
    if len(mask.index) > 0:
        dfy = dfy.append(mask)
...
def StatutoryEffectiveDate_RE_Check(dfx, dfy):
    mask = dfx.loc[(dfx["StatutoryEffectiveDate"].isnull()) |
        ...
        ReasonRemoved='Incomplete StatutoryEffectiveDate').reset_index()
    if len(mask.index) > 0:
        dfy = dfy.append(mask)
...
def StatutoryEndDate_RE_Check(dfx, dfy):
    mask = dfx.loc[(dfx["StatutoryEndDate"].str.contains(','))].assign(
        ReasonRemoved='Incomplete StatutoryEndDate').reset_index()
    if len(mask.index) > 0:
        dfy = dfy.append(mask)
...
def RegulatoryOverlayTypeCV_RE_Check(dfx, dfy):
    mask = dfx.loc[dfx["RegulatoryOverlayTypeCV"].str.len() > 100].assign(
        ReasonRemoved='Incomplete RegulatoryOverlayTypeCV').reset_index()
    if len(mask.index) > 0:
        dfy = dfy.append(mask)
