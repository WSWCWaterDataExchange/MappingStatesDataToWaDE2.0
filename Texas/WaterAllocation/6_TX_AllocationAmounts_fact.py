...
def retrieveWaterSourceUUID(colrowValueA, colrowValueB, df_watersources):
    if (colrowValueA == '' and colrowValueB == '') or (pd.isnull(colrowValueA) and pd.isnull(colrowValueB)):
        ...
        ml = df_watersources.loc[(df_watersources['WaterSourceName'] == colrowValueA) & (
                    df_watersources['WaterSourceTypeCV'] == colrowValueB), 'WaterSourceUUID']
...
outdfpurgeAA = outdf.loc[(outdf["AllocationAmount"].isnull()) | (outdf["AllocationAmount"] == '') & (
            (outdf["AllocationMaximum"].isnull()) | (outdf["AllocationMaximum"] == ''))]
outdfpurge = outdfpurge.append(outdfpurgeAA)
if len(outdfpurgeAA.index) > 0:
    dropIndex = outdf.loc[(outdf["AllocationAmount"].isnull()) | (outdf["AllocationAmount"] == '') & (
                (outdf["AllocationMaximum"].isnull()) | (outdf["AllocationMaximum"] == ''))].index
...
outdf100 = outdf.groupby('AllocationNativeID', sort=False).agg(
...
dfpurge = pd.DataFrame(columns=columns)  # purge DataFrame
...
#double checking that AllocationAmount can be a null value.
...
# AllocationAmount_float_Yes
mask = outdf100.loc[ (outdf100['AllocationAmount'].str.contains(',') == True) ].assign(ReasonRemoved='Bad AllocationAmount').reset_index()
if len(mask.index) > 0:
    dfpurge = dfpurge.append(mask)
    dropIndex = outdf100.loc[ (outdf100['AllocationAmount'].str.contains(',') == True) ].index
    outdf100 = outdf100.drop(dropIndex)
    outdf100 = outdf100.reset_index(drop=True)
...
#double checking that AllocationMaximum can be a null value.
...
# AllocationMaximum_float_Yes
mask = outdf100.loc[ (outdf100['AllocationMaximum'].str.contains(',') == True) ].assign(ReasonRemoved='Bad AllocationMaximum').reset_index()
if len(mask.index) > 0:
    dfpurge = dfpurge.append(mask)
    dropIndex = outdf100.loc[ (outdf100['AllocationMaximum'].str.contains(',') == True) ].index
    outdf100 = outdf100.drop(dropIndex)
    outdf100 = outdf100.reset_index(drop=True)
...
# AllocationPriorityDate_bigint_-
...
#double checking that AllocationTypeCV can be a null value.
...
# AllocationTypeCV_nvarchar(250)_Yes
mask = outdf100.loc[ (outdf100["AllocationPriorityDate"].str.contains(',') == True) |
                     (outdf100["AllocationTypeCV"].str.len() > 250) ].assign(ReasonRemoved='Bad AllocationTypeCV').reset_index()
if len(mask.index) > 0:
    dfpurge = dfpurge.append(mask)
    dropIndex = outdf100.loc[ (outdf100["AllocationPriorityDate"].str.contains(',') == True) |
                     (outdf100["AllocationTypeCV"].str.len() > 250) ].index
    outdf100 = outdf100.drop(dropIndex)
    outdf100 = outdf100.reset_index(drop=True)
