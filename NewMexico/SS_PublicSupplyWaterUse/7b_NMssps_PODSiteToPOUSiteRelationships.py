...
#Purpose: To create NM site specific public supply water use site POD and POU relation information and populate dataframe for WaDEQA 2.0.
...
dfrecord_1 = pd.DataFrame(columns=neededColumnList, index=dfrecord.index)
dfrecord_1 = dfrecord[neededColumnList].drop_duplicates().reset_index(drop=True)

# Explode record data by SiteUUID
dfrecord_1 = dfrecord_1.assign(SiteUUID=dfrecord_1['SiteUUID'].str.split(',')).explode('SiteUUID').reset_index(drop=True)

# Merging dataframes into one, using left-join.
# merge by SiteUUID for POD sites
# merge again with CommunityWaterSupplySystem = SiteName for POU sites.
dfrecord_2a = pd.merge(dfrecord_1, dfsites[['SiteUUID', 'PODorPOUSite']], on='SiteUUID', how='left')
dfrecord_2b = pd.merge(dfrecord_1, dfsites[['SiteName', 'PODorPOUSite']], left_on='CommunityWaterSupplySystem', right_on='SiteName', how='left')
frames = [dfrecord_2a, dfrecord_2b]
dfrecord_2 = pd.concat(frames).reset_index(drop=True)
