...
RegulatoryOverlayUUIDdict = pd.Series(df_regulatoryoverlays.RegulatoryOverlayUUID.values, index = df_regulatoryoverlays.RegulatoryName).to_dict()
...
outdf['DataPublicationDate'] = "08/12/2021"
...
outdf['RegulatoryOverlayUUID'] = df_DM.apply(lambda row: retrieveRegulatoryOverlayUUID(row['in_RegulatoryName']), axis=1)

print("ReportingUnitUUID")  # Using ReportingUnitName to identify ID
outdf['ReportingUnitUUID'] = df_DM.apply(lambda row: retrieveReportingUnitsUUID(row['in_ReportingUnitName']), axis=1)
