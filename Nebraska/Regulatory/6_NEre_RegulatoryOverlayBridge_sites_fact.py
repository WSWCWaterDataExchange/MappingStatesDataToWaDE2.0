...
# Purpose: To extract NE regulatory overlay bridge to site information and populate a dataframe WaDEQA 2.0.
# Notes: 1) asdf
...
M_fileInput = "RawinputData/NE_Sites_RegulatoryOverlay_Bridge_input.csv"  # ArcGIS Pro generated input file.
...
RegulatoryOverlayUUIDdict = pd.Series(dr_regover.RegulatoryOverlayUUID.values, index = dr_regover.RegulatoryName).to_dict()
...
df_regToSite['RegulatoryOverlayUUID'] = df_DM.apply(lambda row: retrieveRegulatoryOverlayUUID(row['AreaName']), axis=1)
...
df_sites = pd.merge(df_sites, df_regToSite[['SiteUUID', 'RegulatoryOverlayUUID']], left_on='SiteUUID', right_on='SiteUUID', how='left')
