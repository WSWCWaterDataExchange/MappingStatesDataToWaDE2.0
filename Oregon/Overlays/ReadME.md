...
This readme details the process that was applied by the staff of the [Western States Water Council (WSWC)](http://wade.westernstateswater.org/) to extracting overlay area data, made available by the [Oregon Water Resources Department](https://www.oregon.gov/OWRD/access_Data/Pages/Data.aspx), for inclusion into the Water Data Exchange (WaDE) project.  WaDE enables states to share data with each other and the public in a more streamlined and cost-effective way.
...
- **2_ORov_CreateWaDEInputFiles.ipynb**: used to create the WaDE input csv files: date.csv, organization.csv, reportingunits.csv, regulatoryoverlays.csv, regulatoryreportingunits.csv, sites.csv, etc.
- **3_ORov_WRSiteRegulatoryID.ipynb**: used to pair overlay information to water allocation information using an overlay on water allocation site information within the boundaries of the regulation.
- **4_ORov_WaDEDataAssessmentScript.ipynb**: used to evaluate the WaDE input csv files.
...
---------- | ---------- | ------------ | ------------ | ------------
ORov_O1 | Oregon Water Resources Department | - | https://www.oregon.gov/owrd/Pages/index.aspx/ | OR 
...
- Assign state agency data info to the *WaDE ReportingUnits* specific columns.  See *ORov_Overlay Info Schema Mapping to WaDE.xlsx* for specific details.  Items of note are as follows...
    - *ReportingUnitUUID* = "ORov_RU + BASIN_NUM"
    - *EPSGCodeCV* = 4326.
...
---------- | ---------- | ------------ | ------------ | ------------ | ------------ | ------------ | ------------ | ------------
ORov_RU + BASIN_NUM | 4326 | BASIN_NAME | BASIN_NUM | 9.6 | Administrative Basin | 9/22/2021 | OR | -
...
- Assign state agency data info to the *WaDE Water Overlays* specific columns.  See *ORov_Overlay Info Schema Mapping to WaDE.xlsx* for specific details.  Items of note are as follows...
    - *RegulatoryOverlayUUID* = ORov_RO + BASIN_NUM
...
---------- | ---------- | ------------ | ------------ | ------------ | ------------ | ------------ | ------------ | ------------ | ------------ | ------------ | ------------
ORov_RO + BASIN_NAME | Oregon Water Resources Department | administrative rules which establish water management policies and objectives and which govern the appropriation and use of the surface and ground water | BASIN_NAME | BASIN_NUM | Active | - | - | 10/7/1993 | - | Administrative Basin | Surface Water and Groundwater
...
---------- | ---------- | ------------ | ------------ 
9/4/2023 | ORov_O1 | ORov_ROor1 | ORov_RUor1
