...
#Purpose: To extract USBR UCRB agg reporting unit use information and population dataframe for WaDEQA 2.0.
...
os.chdir(workingDir)
fileInput = "RawinputData/P_USBAggUCRB.csv"
...
outdf['ReportingUnitName'] = df['ReportingUnit']

print("ReportingUnitNativeID") # Hardcoded
...
if len(mask.index) > 0:
    dfpurge = dfpurge.append(mask)
    dropIndex = outdf.loc[ (outdf["ReportingUnitUpdateDate"].isnull()) | (outdf["ReportingUnitUpdateDate"] == '') | (outdf["ReportingUnitUpdateDate"].str.contains(',') == True) ].index
    outdf = outdf.drop(dropIndex)
    outdf = outdf.reset_index(drop=True)
