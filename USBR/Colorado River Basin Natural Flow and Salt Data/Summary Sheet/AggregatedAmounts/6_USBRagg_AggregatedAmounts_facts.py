...
#Author: Ryan James
#Purpose: To extract USBR UCRB agg information and population dataframe WaDEQA 2.0.
...
os.chdir(workingDir)
M_fileInput = "RawinputData/P_USBAggUCRB.csv"
...
outdf.MethodUUID = "USBR_Colorado River Basin Natural Flow and Salt Summary Data"

print("OrganizationUUID")  # Hardcoded
outdf.OrganizationUUID = "USBR"

print("ReportingUnitUUID")  # Using ReportingUnit to identify ID
...
outdf['ReportingUnitUUID'] = df_DM.apply(lambda row: retrieveReportingUnits(row['ReportingUnit']), axis=1)

print("VariableSpecificUUID")  # Hardcoded
outdf.VariableSpecificUUID = "USBR_Consumptive Use"

print("WaterSourceUUID")  # Using WaterSourceName to identify ID
WaterSourceUUIDdict = pd.Series(df_watersources.WaterSourceUUID.values, index = df_watersources.WaterSourceName).to_dict()
outdf['WaterSourceUUID'] = df_DM.apply(lambda row: retrieveWaterSourceUUID(row['WaterSource']), axis=1)

print("Amount")
outdf['Amount'] = df_DM['Amount']

print("BeneficialUseCategory")
outdf['BeneficialUseCategory'] = df_DM['BeneficialUseCV']
...
outdf.DataPublicationDate = "06/19/2020"
...
if len(mask.index) > 0:
    dfpurge = dfpurge.append(mask)
    dropIndex = outdf100.loc[ (outdf100["AllocationCropDutyAmount"].isnull()) | (outdf100["AllocationCropDutyAmount"] == '') | (outdf100['AllocationCropDutyAmount'].str.contains(',') == True) ].index
    outdf100 = outdf100.drop(dropIndex)
    outdf100 = outdf100.reset_index(drop=True)
