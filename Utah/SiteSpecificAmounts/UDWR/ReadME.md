...
The following text summarizes the process used by the WSWC staff to prepare and share UDWR's site specific time series water data for inclusion into the Water Data Exchange (WaDE 2.0) project.  For a complete mapping outline, see *UDWR MI_SiteSpecificAmounts Schema Mapping to WaDE_QA.xlsx*.  Six executable code files were used to extract the ADWR's aggregated water budget data from the above mentioned input files.  Each code file is numbered for order of operation.  The first code file (pre-process) was built and ran within [Jupyter Notebooks](https://jupyter.org/), the remaining five code files were built and operated within [Pycharm Community](https://www.jetbrains.com/pycharm/). The last code file *(SiteSpecificAmounts)* is dependent on the previous files.  Those six code files are as follows...

- 0_PreProcessUDWRSiteSpecificData.ipynb
- 1_UDWR_SS_Methods.py
- 2_UDWR_SS_Variables.py
- 3_UDWR_SS_Organizations.py
- 4_UDWR_SS_WaterSources.py
- 5_UDWR_SS_Sites.py
- 6_UDWR_SS_SiteSpecificAmounts_fact.py


***
### 0) Code File: 0_PreProcessUDWRSiteSpecificData.ipynb
...
- Generated WKT from RetailCulinaryWaterServiceAreas.shp file to create *Geometry* WaDE input.  Will match on **Source ID** = **WRID**.
- Export output dataframe as new csv file, *P_MasterUTSiteSpecific.csv*.


***
### 1) Code File: - 1_UDWR_SS_Methods.py
...
---------- | ---------- | ------------
UDWR_Water Use Data | Surface Ground Water | Measured


***
### 2) Code File: 2_UDWR_SS_Variables.py
...
---------- | ---------- | ------------ | ------------
UDWR_Site Specific Withdrawal | 1 | Year | AFY


***
### 3) Code File: 3_UDWR_SS_Organizations.py
...
---------- | ---------- | ------------ | ------------
UDWR | AUtah Division of Water Rights | James Greer | https://waterrights.utah.gov/


***
### 4) Code File: 4_UDWR_SS_WaterSources.py
...
- Assign state agency data info to the *WaDE WaterSources* specific columns.  See *UDWR MI_SiteSpecificAmounts Schema Mapping to WaDE_QA.xlsx* for specific details.  Items of note are as follows...
...
***
### 5) Code File: 5_UDWR_SS_Sites.py
...
- Assign state agency data info to the *WaDE Site* specific columns.  See *UDWR MI_SiteSpecificAmounts Schema Mapping to WaDE_QA.xlsx* for specific details.  Items of note are as follows...
    - *County* = **County**.
...
***
### 6) Code File: 6_UDWR_SS_SiteSpecificAmounts_fact.py
...
- Assign state agency data info to the *WaDE Water Allocations* specific columns.  See *UDWR MI_SiteSpecificAmounts Schema Mapping to WaDE_QA.xlsx* for specific details.  Items of note are as follows...
    - Extract *MethodUUID*, *VariableSpecificUUID*, *OrganizationUUID*, *WaterSourceUUID*, & *SiteUUID* from respective input csv files. See code for specific implementation of extraction.
    - *Amount* = **Total Use**.
    - *Geometry* = generated WKT geometry from P_Geometry.csv.  See *0_PreProcessUDWRSiteSpecificData.ipynb* for specific on generation.
    - *ReportYearCV* = **History Year**.
    - *TimeframeStart* = 01/01/+**History Year**.  See *0_PreProcessUDWRSiteSpecificData.ipynb* for specific on generation.
    - *TimeframeEnd* = 12/31/+**History Year**.  See *0_PreProcessUDWRSiteSpecificData.ipynb* for specific on generation.
- Perform error check on output dataframe.
